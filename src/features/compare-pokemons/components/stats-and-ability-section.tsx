    ...
    <>
      <Card heading="Base Stats">
        {STATS_LABELS.map((label, idx) => {
          const baseStat = (pokemon.pokemon_v2_pokemonstats[idx] as unknown as { base_stat: number }).base_stat;
          return (
            <Fragment key={label}>
      ...
      <Card heading="Abilities" className="flex-grow">
        <ul className="list-disc space-y-2 pl-5 text-sm">
          {pokemon.pokemon_v2_pokemonabilities.map(({ pokemon_v2_ability }:any) => (
