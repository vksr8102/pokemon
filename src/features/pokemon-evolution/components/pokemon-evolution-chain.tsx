...
export default function PokemonEvolutionChain({ evolution, className }: Props) {
  // Sort the evolution array by minLevel
  const sortedEvolution = [...evolution].sort((a, b) => {
    const levelA = a.minLevel || 0; // Default to 0 if minLevel is undefined
    const levelB = b.minLevel || 0; // Default to 0 if minLevel is undefined
    return levelA - levelB; // Ascending order
  });

  return (
    <ul
      key={sortedEvolution.map((pokemon) => pokemon.id).join('')}
      className={clsx('pokemon-evolution', className)}
    >
      {sortedEvolution.map((pokemon, idx) => {
